 ...
 * - finding a gearbox part and picking it up.
 */
...
*/
    ...
    cout << "Enter 1 to look for GEARBOX_TOP, 2 to look for GEARBOX_BOTTOM" << endl;
    cin >> ans;

    if(ans == 1){
        ROS_WARN("using GEARBOX_TOP code for perception!");        
        partCode = part_codes::part_codes::GEARBOX_TOP;
    }
    else{
        ROS_WARN("using GEARBOX_BOTTOM code for perception!");        
        partCode = part_codes::part_codes::GEARBOX_BOTTOM;
    }

    ROS_INFO("Attempting to locate gearbox part(s) on table");
