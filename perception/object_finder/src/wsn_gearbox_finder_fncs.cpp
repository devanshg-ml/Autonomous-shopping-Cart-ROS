...
//FIX ME!!
const float MIN_X_GEARBOX = 0.5; //include points starting 0.4m in front of robot 
...
const float MIN_DZ_GEARBOX = 0.01; //0.01; //box filter from this height above the table top
const float MAX_DZ_GEARBOX = 0.05; //consider points up to this height above table top
const double TABLE_GRASP_CLEARANCE_GEARBOX= 0.01;

const double MAX_EVAL_UPPER_BOUND_GEARBOX = 500000.0; //TUNE ME!
const double MAX_EVAL_LOWER_BOUND_GEARBOX = 5000.0;
const double EVAL_RATIO_UPPER_BOUND_GEARBOX = 15.0;
const double EVAL_RATIO_LOWER_BOUND_GEARBOX = 3.0;
//        float min_blob_avg_ht= MIN_BLOB_AVG_HEIGHT, float min_blob_pixels=MIN_BLOB_PIXELS);

const double GEARBOX_MIN_BLOB_AVG_HEIGHT = 80; //110;  //try 110 for real sys
const double GEARBOX_MIN_BLOB_PIXELS = 400;
...
//            vector<float> &avg_z_heights, vector<float> &npts_blobs,  vector<geometry_msgs::PoseStamped> &object_poses) {
   ...
   //convert point cloud to top-down 2D projection for OpenCV processing
    ...
    for (int iblob=0;iblob<nblobs;iblob++) {
        if (max_evals_[iblob]>MAX_EVAL_UPPER_BOUND_GEARBOX || max_evals_[iblob]<MAX_EVAL_LOWER_BOUND_GEARBOX) {
            ROS_WARN("max eval = %f is out of range for blob %d",max_evals_[iblob],iblob);
               ...
               ROS_INFO("blob %d max_eval = %f is in range; eval ratio max/mid = %f",iblob,max_evals_[iblob],eval_ratio);
                if (eval_ratio>EVAL_RATIO_UPPER_BOUND_GEARBOX || eval_ratio<EVAL_RATIO_LOWER_BOUND_GEARBOX) {
...
//            vector<float> &avg_z_heights, vector<float> &npts_blobs,  vector<geometry_msgs::PoseStamped> &object_poses) {
   ...
   //convert point cloud to top-down 2D projection for OpenCV processing
    ...
    blob_finder(x_centroids_wrt_robot, y_centroids_wrt_robot, avg_z_heights, npts_blobs, viable_labels_,GEARBOX_MIN_BLOB_AVG_HEIGHT,GEARBOX_MIN_BLOB_PIXELS);
    //
